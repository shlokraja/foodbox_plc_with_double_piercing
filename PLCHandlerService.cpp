...
extern void *WriteVarToPLCTATA(PLC *pPLC, char *pszVarName, char *pszVal, int iLen);
extern void *WriteVarToPLCTATA2(PLC *pPLC, char *pszVarName, char *pszVal, int iLen);
...
void DispenseItemFromList(pItemDispenseData pItem)
{
		...
		//TATA Machine Change
		char szStubTATA[1024] = {0};
		strcpy(szStubTATA, pItem->szOrderStub);

		char *pszBarCodebarcodepart = substr(szStubTATA, 0, 49);
		
		sprintf(szMsg, "BarCodebarcodepart - sending [%s] barcodepart to dispenser", pszBarCodebarcodepart);
		DoLog(szMsg, 1);


		char *pszBarCodeDetailPart = substr(szStubTATA, 49, 58);
		sprintf(szMsg, "pszBarCodeDetailPart - sending [%s] BarCodeDetailPart to dispenser", pszBarCodeDetailPart);
		DoLog(szMsg, 1);
		...
		//WriteVarToPLC(g_pOrderPLC, g_CompInfo.szOrderVar, szStub, strlen(szStub));

		//TATA
		WriteVarToPLCTATA(g_pOrderPLC, g_CompInfo.szOrderVarbarcodepart, pszBarCodebarcodepart, strlen(pszBarCodebarcodepart));
		WriteVarToPLCTATA2(g_pOrderPLC, g_CompInfo.szOrderVardetailpart, pszBarCodeDetailPart, strlen(pszBarCodeDetailPart));
...
void InitializeCompartmentInfo()
{
	...
	else
	{
		...
		strcpy(pCurr->szOrderVarbarcodepart, d1MLstringPlaceOrderbarcode);
		strcpy(pCurr->szOrderVardetailpart, d1MLstringPlaceOrderdetails);
